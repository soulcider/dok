...
public class HttpLoadRunner {
    ...
    class Caller implements Callable<String> {
        ...
        public Caller(String name, int clickCount, long interval) {
            this.name = name;
            ...
            */
            for (int j = 0; (clickCount < 0 ? true : j < clickCount); j++) {
                ...
                if(status >= 200 && status < 300) {
                    ...
                    System.err.println("     - URLConnector Error");
    ...
    public void execute(int userCount, int clickCount, long interval) {
        ...
        for (int i = 0; i < userCount; i++) {
            Caller caller = new Caller(String.format("TASK%d", i + 1), clickCount, interval);
            caller.init();
            tasks.add(caller);
        }
